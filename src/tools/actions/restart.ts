...
export async function handler(input: {
  ...
  try {
    ...
    if (!policyCheck.allowed) {
      logMutationAudit('restart', input.resource_uuid, input.resource_type, 'denied', policyCheck.reason);
    ...
    if (!scopeCheck.allowed) {
      logMutationAudit('restart', input.resource_uuid, input.resource_type, 'denied', scopeCheck.reason);
      return policyDeniedResponse(scopeCheck.reason!, startTime);
    }

    const prodCheck = checkProductionMutation(config, input.environment_name, 'restart');
    if (!prodCheck.allowed) {
      logMutationAudit('restart', input.resource_uuid, input.resource_type, 'denied', prodCheck.reason);
      return policyDeniedResponse(prodCheck.reason!, startTime);
    }

    await client.restartApplication(input.resource_uuid);
    const durationMs = Date.now() - startTime;

    logMutationAudit('restart', input.resource_uuid, input.resource_type, 'allowed');
    logger.info({ resourceUuid: input.resource_uuid, durationMs }, 'Restart triggered');

    const content = JSON.stringify(
      {
        ok: true,
        summary: 'Restart triggered',
        data: { resource_uuid: input.resource_uuid, status: 'restarting' },
        meta: { durationMs },
      },
      null,
      2,
    ...
    const coolifyError =
      error instanceof CoolifyError
        ? error
        : new CoolifyError('Restart failed', 'UPSTREAM_ERROR', 500, false);

    logMutationAudit('restart', input.resource_uuid, input.resource_type, 'error', coolifyError.message);
